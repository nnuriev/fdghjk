...
if [[ $# -ne 2 ]]; then
  echo "usage: analyze-course-visuals-wait.sh COURSE_ID PARALLEL" >&2
  exit 64
fi
...
seq 1 "$(jq 'length' "$metadata")" |
  xargs -n 1 -P "$parallel" bash -c '
    ...
    if [[ "$title" =~ (Вопросы[[:space:]]+и[[:space:]]+ответы|Q\&A|QA-сессия|Созвон) ]]; then
      interval=90
    elif [[ "$title" =~ (Разбор|Практик|Проектирован|реализац|ошиб|Lock-free|Алгоритм|задач|Worker|Batcher|Cache|домашн|Код|код|Mutex|канал|горути|слайс|map|Map|строк|Defer|Append) ]]; then
      interval=30
    fi
